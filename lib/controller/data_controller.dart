...
class DataController extends GetxController{
  ...
  DocumentSnapshot? myDocument2;



  var allUsers  = <DocumentSnapshot>[].obs;
  var alltrainers  = <DocumentSnapshot>[].obs;
  var allrequests  = <DocumentSnapshot>[].obs;
  var filteredtrainers  = <DocumentSnapshot>[].obs;
  var allTrainerrequests  = <DocumentSnapshot>[].obs;
  var filteredtrainersrequests  = <DocumentSnapshot>[].obs;
  var filterdrequests  = <DocumentSnapshot>[].obs;
 ...
 Future<bool> createEvent(Map<String,dynamic> eventData)async{
  ...
  gettrainers(){
    isUsersLoading(true);
    FirebaseFirestore.instance.collection('users').where('isTrainer', isEqualTo: 'trainer').snapshots().listen((event) {
      alltrainers.value = event.docs;
      filteredtrainers.value.assignAll(alltrainers);
      isUsersLoading(false);
    });
  }
  ...
  getrequests(){
    isUsersLoading(true);
    FirebaseFirestore.instance.collection('requests').snapshots().listen((event) {
      allrequests.value = event.docs;
      filterdrequests.value.assignAll(allrequests);
      isUsersLoading(false);
    });
  }
  getTrainerrequests(){
    isUsersLoading(true);
    FirebaseFirestore.instance.collection('Trainer_requests').snapshots().listen((event) {
      allTrainerrequests.value = event.docs;
      filteredtrainersrequests.value.assignAll(allTrainerrequests);
      isUsersLoading(false);
    });
  }
